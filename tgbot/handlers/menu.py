...
async def list_subcategories(callback: CallbackQuery, category: str, **kwargs):
    markup = await subcategories_keyboard(
        callback.bot.get("items_api"), category
    )
...
async def list_items(
    ...
    markup = await items_keyboard(
        callback.bot.get("items_api"), category, subcategory
    )
    await callback.message.edit_text(
        text="Дивись, що у нас є", reply_markup=markup
    )
...
async def buy_item(
    callback: CallbackQuery, item_id: str, state: FSMContext, **kwargs
):
