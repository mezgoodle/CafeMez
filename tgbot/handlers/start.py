...
from aiogram.dispatcher.filters.builtin import ChatTypeFilter, CommandStart
...
@dp.message_handler(
    CommandStart(deep_link=compile(r"^[0-9]{3,15}$")),
...
async def register_referral(message: Message):
    ...
    if status == 201:
        ...
        return await message.answer(
            "Невідома помилка. Будь ласка, зверніться до адміністратора"
        )
...
async def cmd_start(message: Message):
    return await message.answer(
        "Вітаємо! Щоб побачити усі доступні команди, введіть команду:\n"
