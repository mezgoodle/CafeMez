...
from tgbot.misc.backend import Item, Order, Referral, Restaurant, User
from tgbot.misc.invoices.invoice import Item as ItemInvoice
from tgbot.misc.storage import Storage


@dp.message_handler(
    lambda message: not message.text.isdigit() and int(message.text) <= 0,
    state=["item_amount", "change_amount"],
)
async def item_amount_handler(message: Message):
    return await message.reply(
        f'Введіть кількість товару у {hbold("цифрах")} та більше нуля'
    )
...
async def item_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Ви успішно додали товар до корзини! Щоб її переглянути, натисніть /cart"
    )
...
async def apply_purchase(callback_query: CallbackQuery, state: FSMContext):
    ...
    text = (
        "Чи бажаєте ви доставку замовлення?"
...
async def shipping_address_as_location(message: Message, state: FSMContext):
    ...
    restaurant_name, shipping_price = await api.get_shipping_price(
        restaurant_location, message.bot
    )
    if shipping_price > 1000:
        return await message.answer(
            "Ви знаходитесь поза зоною доставки. Оберіть ближче місце або самовивіз з ресторану."
        )
    await state.update_data(
        shipping_price=round(shipping_price),
        shipping_address_name=restaurant_name,
        ...
        if status == 201:
            ...
            if payment_method == "CD":
                ...
                if "detail" in user.keys():
                    ...
                    if discount := await ref_api.get_discount(
                ...
                await message.answer(
                    "Ваше замовлення прийнято! Оплатіть його будь ласка."
                )
                return await bot.send_invoice(
                    message.from_user.id, **order_invoice.generate_invoice()
                )
            return await message.answer(
                "Ваше замовлення прийнято!. Щоб побачити свої замовлення, натисніть /my_orders"
            )
