...
from tgbot.misc.backend import Item, Order, User
...
async def show_restaurants(message: Message, state: FSMContext) -> Message:
    ...
    return await message.answer(
        "Оберіть ресторан, у якому хочете забронювати місце",
        reply_markup=markup,
    )
...
async def enter_email(message: Message, state: FSMContext) -> Message:
    email = message.text
    api: User = message.bot.get("users_api")
    data, status = await api.create_user(
        message.from_user.username, email, message.from_user.id
    )
    await state.finish()
    if status == 201:
        return await message.answer("Ви успішно зареєстровані")
    return await message.answer(
        "Виникла помилка при реєстрації. Зверніться до служби підтримки"
    )
...
async def show_my_ref(message: Message) -> Message:
    ...
    return await message.answer(
        "Ви не зареєстровані в системі. Для цього введіть команду /register"
    )
...
async def show_cart(message: Message):
    ...
    text = (
        f'- Щоб побачити продукт, натисніть на його {hbold("назву")}\n'
...
async def show_faq(message: Message):
    return await message.answer(
        f'{hbold("Як працює система знижок?")}\n\n'
        ...
        f"Знижки працюють завжди, якщо ви користувались реферальним посиланням"
    )
