...
async def answer_credentials(message: Message, state: FSMContext) -> Message:
    ...
    try:
        ...
        return await message.reply(
            f'{hbold("Перегляньте будь ласка налаштування приватності")}'
        )
...
@dp.message_handler(
    state="cafe_coords", content_types=ContentType.LOCATION, is_admin=True
)
...
async def answer_cafe_name(message: Message, state: FSMContext) -> Message:
    ...
    response = await api.create_restaurant(
        message.text, data["latitude"], data["longitude"]
    )
    if response:
        return await message.answer(
            f"Дякую! Ресторан {hbold(message.text)} доданий до бази даних."
        )
    return await message.reply(
        "Щось пішло не так. Зверніться до головного адміністратора."
    )
...
async def update_places_in_restaurant(
    ...
    return await call.message.edit_text(
        "Виникла помилка. Зверніться до головного адміністратора"
    )
