...
@dp.message_handler(
    lambda message: message.text.isdigit() and float(message.text) > 0,
    state=Item.price,
)
...
async def answer_photo(
    message: Message, state: FSMContext, subcategories: list
):
    ...
    return await message.reply(
        "Оберіть категорію товару", reply_markup=keyboard
    )


@dp.message_handler(state=Item.subcategory)
async def answer_subcategory(
    message: Message, state: FSMContext, subcategories: list
):
    ...
    await message.answer_photo(
        data["photo"],
        caption=f'Назва: {hbold(data["name"])}\n'
...
@dp.message_handler(
    lambda message: message.text in ["Так", "Ні"], state=Item.approval
)
