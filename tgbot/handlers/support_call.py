...
from tgbot.keyboards.inline.support_keyboard import (
    cancel_support,
    create_keyboard,
)
...
async def ask_support_call(message: Message) -> Message:
    text = (
        "Хочете зв'язатись із оператором техпідтримки? Натисніть кнопку нижче!"
    )
...
async def send_to_support_call(
    ...
    await call.message.edit_text(
        "Ви звернулися до техпідтримки! Очікуйте відповідь оператора"
    )
    ...
    if not support_id:
        ...
        return await call.message.edit_text(
            "На жаль, немає доступних операторів для зв'язку!"
        )
    ...
    return await bot.send_message(
        support_id,
        f"З вами хоче зв'язатись користувач {call.from_user.full_name}!",
...
async def answer_support_call(
    ...
    if str(await user_state.get_state()) != "wait_in_support":
        return await call.message.edit_text(
            "На жаль, користувач вже передумав!"
        )
    ...
    new_second_message = await bot.send_message(
        second_id,
...
async def not_supported(message: Message, state: FSMContext) -> Message:
    ...
    return await message.answer(
        "Дочекайтесь відповіді оператора або відміність сеанс!",
        reply_markup=keyboard,
    )
    ...
    if await second_state.get_state() is not None:
        ...
        if int(second_id) == call.from_user.id:
            ...
            await bot.send_message(
                user_id, "Користувач завершив сеанс техпідтримки!"
            )
    await state.reset_state()
