...
async def send_to_support(
    ...
    return await call.message.answer(
        "Надішліть ваше повідомлення, яким ви хочете поділитись"
    )


@dp.message_handler(
    state="wait_for_support_message", content_types=ContentTypes.ANY
)
async def get_support_message(message: Message, state: FSMContext) -> Message:
    ...
    await bot.send_message(
        second_id,
        f"Вам надійшов лист! Щоб відповісти, натисніть кнопку нижче!",
    )
